  ...
  while thrr:
    for item in mgdb.get_collection("project").find({}):
      ...
      if not path.exists(follow_symlinks=False):
        ...
        pass
      # print(item["project"],item["categories"],item["name"])
      # print(pathlib.Path("D:/00 PROJELER/").joinpath(item["project"]).joinpath("/".join(item["categories"]),item["name"]).exists)
      if not thrr:
        break
      # time.sleep(1)
  ...
  while thrr:
    ...
    for item in pathlib.Path("D:/00 PROJELER/").rglob("*.pdf"):
      ...
      if fobj:
        ...
        # print(mgdb.get_collection("project").insert_one(obje))
      if not thrr:
        break
      # print(mgdb.get_collection("project").find_one({ "pattern": obje["ptrn"], "name": obje["name"], "project": obje["proj"], "categories": obje["cats"].split(":")}))
      # outs.append(obje)
